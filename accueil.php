...
// Boucle qui permet d'inserer le tweet dans la db si le bouton deadbirder a été pressé et qui vide les données accumulés dans le $_POST juste apres


// Requete qui permet de recuperer la pp de l'utilisateur connecté pour l'afficher a coté du quoi de neuf
$sql = "SELECT avatar FROM users where id=$utilisateur";
...
<div class="col-6">
            ...
            <div class="container">
                ...
                if (isset($_SESSION['id'])) {
                    ...
                    <div class="row">
                        ...
                        <div class="col-2">
                            <img class="rounded-circle bg-light mt-1" src="<?= $result['avatar'] ?>" alt="" width="50px"
                                height="50px">
                        </div>
                        <div class="col-10">

                            <form method="post" action="redirection.php?url=accueil.php?page=8">
                                <div class="emoji-picker-container">
                                    <input type="text" name="tweet_content" id="emoji-input" size=""
                                        placeholder="Quoi de neuf ?" data-emojiable="true" autocomplete="off">
                                </div>
                                ...
                                <div class="row button-twett mt-2 mb-2 pt-2">
                                    ...
                                    <div class="col">
                                        <button type="submit" name="valider" id="boutonique">Deadbirder</button>
                                    </div>
    ...
    $query->execute();
    $nombre = $query->fetch();
    if ($page > $nombre['nombre']) {
        $page = $nombre['nombre'];
    }
    // requete qui va recuperer toutes les infos pour afficher le tweet comme il faut
    $sql = "SELECT users.name,username,message,avatar,date_send,tweet.id as lid,id_reply_tweet,id_user,id_retweet FROM tweet left join users on tweet.id_user=users.id where id_reply_tweet is null order by date_send desc limit $page";
    $query = $conn->prepare($sql);
    $query->execute();
    $result = $query->fetchAll();
    // --------------------------------------------------------------------
    $query = "SELECT users.name,username,message,avatar,date_send,tweet.id as lid,id_reply_tweet,id_user,id_retweet FROM tweet left join users on tweet.id_user=users.id where id_reply_tweet is null order by date_send desc limit $page";
                    ...
                    <div class="row">
                        ...
                        <div class="col-10">
                            ...
                            <div class="inter">
                                ...
                                if ($isrt['test'] == 0) {
                                    ?>
                                    <form method="post" action="redirection.php?id_tweet=<?= $nbrlike ?>&url=accueil.php?page=8">
                                ...
                                if ($islike['test'] == 0) {
                                    ?>
                                    <form method="post" action="redirection.php?id_tweet=<?= $id ?>&url=accueil.php?page=8">
                        ...
                        <div class="col-10">
                            ...
                            <div class="inter">
                                ...
                                if ($islike['test'] == 0) {
                                    ?>
                                    <form method="post"
                                        action="redirection.php?id_tweet=<?= $value['lid'] ?>&url=accueil.php?page=8">
                                        <button type="submit" name="retweet"><i id="deathtwett" class="fa-solid fa-skull">
                                                <?= $resultat['test'] ?>
                                            </i></button>
                                    </form>
