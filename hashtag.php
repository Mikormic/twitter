...
<body>
    <div class="col-6">
        <div class="d-flex justify-content-between w-100">
            ...
            $sql = "SELECT users.name,username,message,avatar,date_send,tweet.id as lid,id_reply_tweet,id_user,id_retweet FROM tweet left join users on tweet.id_user=users.id where tweet.message like '%#$tag%' order by date_send desc";
            $query = $conn->prepare($sql);
            $query->execute();
            $result = $query->fetchAll();
            // --------------------------------------------------------------------
            $query = "SELECT users.name,username,message,avatar,date_send,tweet.id as lid,id_reply_tweet,id_user,id_retweet FROM tweet left join users on tweet.id_user=users.id where tweet.message like '%#$tag%' order by date_send desc";
